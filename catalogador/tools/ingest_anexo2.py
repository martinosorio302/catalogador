...
def resolve_valorizacion(
    valor_serie: str, ag: int, ap: int, oaa: int
) -> Dict[str, Any]:
    total = ag + ap + oaa
...
RX_ENTIDAD = re.compile(
    r"^\s*2\.\s*Nombre\s+de\s+la\s+entidad:\s*(.+?)\s*$", re.IGNORECASE
)
...
def export_artifacts(
    parsed: List[Dict[str, Any]], pdf_path: Path, outdir: Path
) -> None:
...
def main(argv: List[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--pdf", "-p", help="Path to ESSALUD-PCD-ANEXO-2-TABLA.pdf", default=None
    )
    parser.add_argument(
        "--outdir",
        "-o",
    ...
    parser.add_argument(
        "--force", "-f", action="store_true", help="Force re-extraction"
    )
