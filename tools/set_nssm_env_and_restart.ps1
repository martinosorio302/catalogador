...
try { Stop-Service -Name $ServiceName -Force -ErrorAction Stop } catch { Write-Warning "Stop-Service: $($_.Exception.Message)" }
Start-Sleep -Seconds 2
try { Start-Service -Name $ServiceName -ErrorAction Stop; Write-Host 'Service started' } catch { Write-Warning "Start-Service: $($_.Exception.Message)" }
...
try {
    ...
    if ($appParams -match '--host\s+0\.0\.0\.0') {
        ...
        Start-Sleep -Seconds 2
        try { Start-Service -Name $ServiceName -ErrorAction Stop } catch { Write-Warning "Start-Service: $($_.Exception.Message)" }
    ...
    Write-Warning "Could not inspect/update AppParameters: $($_.Exception.Message)"
}
...
try {
    ...
    Write-Warning "Failed to force AppParameters: $($_.Exception.Message)"
}
