...
def find_pdf(hint: str | None) -> Path:
    ...
    for c in candidates:
        if c.exists():
            return c.resolve()
    raise FileNotFoundError("No PDF found. Provide --pdf or place ESSALUD-PCD-ANEXO-2-TABLA.pdf on Desktop/Downloads.")
...
def run_pdftotext(pdf: Path, out_txt: Path) -> None:
    ...
    if shutil.which("pdftotext"):
        cmd = ["pdftotext", "-layout", "-enc", "UTF-8", "-nopgbrk", str(pdf), str(out_txt)]
        proc = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        if proc.returncode != 0:
            raise RuntimeError(f"pdftotext failed: {proc.returncode} {proc.stderr.decode('utf-8',errors='replace')}" )
        return
    ...
    with pdfplumber.open(str(pdf)) as pdff, out_txt.open('w', encoding='utf-8') as outfh:
...
def resolve_valorizacion(valor_serie: str, ag: int, ap: int, oaa: int) -> Dict[str, Any]:
    ...
    if valor_serie.upper() == 'PERMANENTE':
        ...
        destino = 'Archivo Central (opcional/intermedio)' if total >= 10 else 'Archivo de Gestion/Periferico'
        return {
...
RX_ASUNTO = re.compile(r'^\s*3\.\s*Asunto\s+Principal\s+de\s+la\s+Serie\s+Documental:\s*(.+?)\s*$', re.IGNORECASE)
RX_ENCABEZADO = re.compile(r'4\.|Codigo.*Titulo.*Valor|Periodo', re.IGNORECASE)
# Row: Nº Ord | Código | Título | Valor | AG | AP | OAA | Total
RX_FILA = re.compile(r'^\s*(\d+)\s+([A-Z0-9\/\-]+)\s+(.+?)\s+(PERMANENTE|TEMPORAL)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s*$', re.IGNORECASE)
...
def parse_layout_text(txt_path: Path) -> List[Dict[str, Any]]:
    ...
    for i, line in enumerate(lines, start=1):
        ...
        if within_table:
            ...
            if re.search(r'^TABLA\s+GENERAL\s+DE\s+RETENCION\s+DE\s+DOCUMENTOS', trim, re.IGNORECASE):
                within_table = False
                continue
...
def export_artifacts(parsed: List[Dict[str, Any]], pdf_path: Path, outdir: Path) -> None:
    ...
    if parsed:
        keys = ['fuente_pdf','pagina_aprox','sector','fondo_documental','unidad_productora','fraccion_documental',
        ...
        tmp_path = None
        try:
            fd, tmp_path = tempfile.mkstemp(prefix=csv_path.name + '.', suffix='.tmp', dir=str(csv_path.parent))
...
"""
    ...
    tmp_path = None
    try:
        fd, tmp_path = tempfile.mkstemp(prefix=schema_path.name + '.', suffix='.tmp', dir=str(schema_path.parent))
    ...
    tmp_path = None
    try:
        fd, tmp_path = tempfile.mkstemp(prefix=seed_path.name + '.', suffix='.tmp', dir=str(seed_path.parent))
        with os.fdopen(fd, 'w', encoding='utf-8') as fh:
            ...
            for r in parsed:
                vals = [
                    ...
                    r['valor_serie'],
                    str(r['retencion_ag']), str(r['retencion_ap']), str(r['retencion_oaa']), str(r['retencion_total']),
                    r['valoracion_decision'], r['valoracion_destino'].replace("'", "''"),
                    r['valoracion_momento'].replace("'", "''"), r['valoracion_justifica'].replace("'", "''"), r['hash_fila']
                ]
                def q(x):
                    try:
                        int(x)
...
def main(argv: List[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description='Ingest ANEXO 2 (PCD EsSalud) and produce normalized artifacts')
    parser.add_argument('--pdf', '-p', help='Path to ESSALUD-PCD-ANEXO-2-TABLA.pdf', default=None)
    parser.add_argument('--outdir', '-o', help='Output directory', default=str(Path.home() / 'Desktop' / 'PCD-EsSalud-OUT'))
