...
def reload_trd_data() -> dict:
    ...
    try:
        ...
        summary['counts'] = {
            ...
            'indice_tokens': len(INDICE_TOKENS) if isinstance(INDICE_TOKENS, list) else 0,
            'inventario_descripcion': len(INVENTARIO_DESCRIPCION) if isinstance(INVENTARIO_DESCRIPCION, dict) else 0,
        }
        if errors:
...
def texto_indice(asunto_unidad: Optional[str], titulo_doc: Optional[str], productor: Optional[str]) -> str:
...
def clasificar_por_tokens(asunto_unidad: Optional[str], titulo_doc: Optional[str], productor: Optional[str]) -> Optional[str]:
...
def buscar_trd_por_asunto_titulo(asunto: str, serie_sugerida: Optional[str] = None) -> Optional[Dict[str, Any]]:
...
def aplicar_reglas_trd(input_obj: Dict[str, Optional[str]]) -> Dict[str, Any]:
    ...
    if input_obj.get('serie') and input_obj.get('asuntoUnidad'):
        por_asunto = buscar_trd_por_asunto_titulo(input_obj.get('asuntoUnidad', ''), input_obj.get('serie', ''))
    code_tok = clasificar_por_tokens(input_obj.get('asuntoUnidad'), input_obj.get('titulo'), input_obj.get('productor'))
